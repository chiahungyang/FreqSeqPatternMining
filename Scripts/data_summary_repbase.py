...
def main():
    ...
    logging.info('Start.')
    seq_len = [len(seq) for seq in sequences('../Data/repbase_arabidopsis_thaliana_TE.fasta')]
    ...
    max_len = max(seq_len)
    num_nc = sum(seq_len)
    with open('../Results/data_summary_repbase.txt', 'w') as fp:
    ...
    fig.savefig('../Results/sequence_length_distribution_repbase.png', dpi=300)
